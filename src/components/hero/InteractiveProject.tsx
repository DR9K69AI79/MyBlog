    ...
    <div className={`w-full max-w-2xl mx-auto space-y-6 overflow-visible relative ${className}`}>
      ...
      {currentProject ? (
        <>
          {/* 桌面端：整体内容区域，为右侧导航留空间 */}
          <div
            className={`${isMobile ? 'w-full' : 'pr-20'} transition-all duration-300 overflow-visible`}
          >
            <motion.div
          ...
          {!isMobile && (
            ...
            >
              ...
              >
                ...
                {/* 图标 */}
                <svg
                  className="relative w-5 h-5 text-primary group-hover:text-accent transition-colors duration-200"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
